...
Copyright 2016-2017 Stijn Sanders
...
Tread-Safe version of TJSONDocument
...
{
  TJSONDocumentThreadSafe class
  the default IJSONDocument implementation
  see also: JSON function
}
  TJSONDocumentThreadSafe = class(TInterfacedObject, IJSONDocument,
...

{
  ...
  TJSONEnumeratorThreadSafe = class(TInterfacedObject, IJSONEnumerator)
  private
    FData:TJSONDocumentThreadSafe;
    FIndex: integer;
  public
    constructor Create(Data: TJSONDocumentThreadSafe);
    destructor Destroy; override;
    function EOF: boolean; stdcall;
    function Next: boolean; stdcall;
    function Get_Key: WideString; stdcall;
...

{
  ...
  TJSONDocArrayBuilderThreadSafe = class(TInterfacedObject, IJSONDocArrayBuilder)
  private
...
{ TJSONDocumentThreadSafe }

procedure TJSONDocumentThreadSafe.AfterConstruction;
begin
  inherited;
...
destructor TJSONDocumentThreadSafe.Destroy;
var
  i:integer;
begin
...
function TJSONDocumentThreadSafe.GetKeyIndex(const Key: WideString;
  var GotIndex:integer;var GotSorted:integer):boolean;
var
  a,b,c,d,x:integer;
begin
...
function TJSONDocumentThreadSafe.Get_Item(const Key: WideString): OleVariant;
var
...
function TJSONDocumentThreadSafe.ReUse(const Key: WideString): OleVariant;
var
  GotIndex,GotSorted:integer;
begin
  EnterCriticalSection(FLock);
...
procedure TJSONDocumentThreadSafe.Set_Item(const Key: WideString;
...
function TJSONDocumentThreadSafe.Parse(const JSONData: WideString): IJSONDocument;
var
  i,l:integer;
...
function TJSONDocumentThreadSafe.JSONToString: WideString;
...
begin
  ...
  try
    ...
    stackLength:=0;
    stackIndex:=0;
    e:=TJSONEnumeratorThreadSafe.Create(Self);
    a:=Null;
    ai:=0;
...
function TJSONDocumentThreadSafe.ToVarArray: OleVariant;
var
  i,l:integer;
begin
  if Self=nil then
...
procedure TJSONDocumentThreadSafe.Clear;
var
  i:integer;
  uu:IUnknown;
  d:IJSONDocument;
...
function TJSONDocumentThreadSafe.NewEnumerator: IJSONEnumerator;
begin
  Result:=TJSONEnumeratorThreadSafe.Create(Self);
end;
...
function JSON:IJSONDocument; //overload;
begin
  Result:=TJSONDocumentThreadSafe.Create as IJSONDocument;
end;
...
var
  ...
  key:WideString;
begin
  d:=TJSONDocumentThreadSafe.Create;//d:=JSON;
  si:=0;
  sl:=0;
  i:=0;
  l:=Length(x);
     ...
     begin
      ...
      if (key<>'') and (key[Length(key)]='{') then
       begin
        ...
        s[si]:=d;
        d:=TJSONDocumentThreadSafe.Create;
...
constructor TJSONEnumeratorThreadSafe.Create(Data: TJSONDocumentThreadSafe);
begin
  inherited Create;
  FData:=Data;
  FIndex:=-1;
...
destructor TJSONEnumeratorThreadSafe.Destroy;
begin
  FData:=nil;
  inherited;
end;

function TJSONEnumeratorThreadSafe.EOF: boolean;
var
  i:integer;
begin
  if FData=nil then
...
function TJSONEnumeratorThreadSafe.Next: boolean;
begin
  if FData=nil then
    Result:=false
  else
...
function TJSONEnumeratorThreadSafe.Get_Key: WideString;
begin
  EnterCriticalSection(FData.FLock);
  try
...
function TJSONEnumeratorThreadSafe.Get_Value: OleVariant;
begin
  EnterCriticalSection(FData.FLock);
  try
    if (FIndex<0) or (FData=nil) or (FIndex>=FData.FElementIndex) then
...
procedure TJSONEnumeratorThreadSafe.Set_Value(const Value: OleVariant);
begin
  EnterCriticalSection(FData.FLock);
  try
    if (FIndex<0) or (FData=nil) or (FIndex>=FData.FElementIndex) then
...
function JSONDocArray: IJSONDocArrayBuilder; overload;
begin
  Result:=TJSONDocArrayBuilderThreadSafe.Create;
end;
...
constructor TJSONDocArrayBuilderThreadSafe.Create;
begin
  inherited Create;
  FItemsCount:=0;
  FItemsSize:=0;
...
destructor TJSONDocArrayBuilderThreadSafe.Destroy;
begin
  DeleteCriticalSection(FLock);
  SetLength(FItems,0);
  inherited;
...
function TJSONDocArrayBuilderThreadSafe.Count: integer;
begin
  EnterCriticalSection(FLock);
  try
    Result:=FItemsCount;
...
procedure TJSONDocArrayBuilderThreadSafe.Clear;
var
  i:integer;
begin
  EnterCriticalSection(FLock);
...
procedure TJSONDocArrayBuilderThreadSafe.LoadItem(Index: integer;
  Doc: IJSONDocument);
begin
  EnterCriticalSection(FLock);
  try
...
function TJSONDocArrayBuilderThreadSafe.Get_Item(Index: integer): IJSONDocument;
begin
  EnterCriticalSection(FLock);
  try
    if (Index<0) or (Index>=FItemsCount) then
...
function TJSONDocArrayBuilderThreadSafe.Add(Doc: IJSONDocument): integer;
begin
  EnterCriticalSection(FLock);
  try
...
function TJSONDocArrayBuilderThreadSafe.AddJSON(const Data: WideString): integer;
begin
  EnterCriticalSection(FLock);
  try
    if FItemsCount=FItemsSize then
...
procedure TJSONDocArrayBuilderThreadSafe.Set_Item(Index: integer;
  Doc: IJSONDocument);
var
  v:WideString;
begin
...
function TJSONDocArrayBuilderThreadSafe.JSONToString: WideString;
var
  i,x,l:integer;
begin
  EnterCriticalSection(FLock);
